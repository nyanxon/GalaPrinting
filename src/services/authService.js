...
// ---------------------------------------------------------------------------
// seedStaffUsers — localStorage path only
// ---------------------------------------------------------------------------
 ...
 * The call-site guard in App.jsx ensures it is never called in backend mode.
 *
 * Creates missing staff accounts and fixes roles on existing ones.
 */
export function seedStaffUsers({
  adminEmail       = "admin@galaprinting.test",
  ownerEmail       = "owner@galaprinting.test",
  cashierEmail     = "cashier@galaprinting.test",
  csEmail          = "cs@galaprinting.test",
  operationalEmail = "operational@galaprinting.test",
  qcEmail          = "qc@galaprinting.test",
  offlineEmail     = "offline@galaprinting.test",
} = {}) {
  let users = loadUsers();

  /**
   * Ensure a staff account exists with the correct role.
   * - If the account doesn't exist → create it.
   * - If it exists but has the wrong role → fix the role.
   * - If it exists with the correct role → no-op.
   */
  const ensure = (role, email, name) => {
    ...
    if (!existing) {
      ...
      // Fix stale role from old seed
      existing.role = role;
    }
  };

  ensure("admin",       adminEmail,       "Super Admin");
  ensure("owner",       ownerEmail,       "Owner");
  ensure("cashier",     cashierEmail,     "Kasir");
  ensure("cs",          csEmail,          "Customer Service");
  ensure("operational", operationalEmail, "Operasional");
  ensure("qc",          qcEmail,          "Quality Control");
  ensure("offline",     offlineEmail,     "Offline Admin");
  saveUsers(users);
}
